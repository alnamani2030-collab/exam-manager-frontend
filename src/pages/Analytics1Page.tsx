...
function buildTeacherAnalytics(assignments: Assignment[]): TeacherAnalyticsRow[] {
  ...
  for (const assignment of assignments) {
    ...
    current.total = current.monitoring + current.reserve + current.review + current.correction;
...
function buildAutoRedistributionSuggestions(rows: TeacherAnalyticsRow[], lang: Lang): TransferSuggestion[] {
  ...
  const taskTypes: TaskType[] = ["INVIGILATION", "RESERVE", "REVIEW_FREE", "CORRECTION_FREE"];
...
function runSelfTests(): void {
  ...
  const tests = [
    {
      name: "aggregates total tasks per teacher",
      pass: rows.find((row) => row.teacher === "A")?.total === 2,
    },
    {
      name: "maps correction tasks correctly",
      pass: rows.find((row) => row.teacher === "B")?.correction === 2,
    },
    {
  ...
  return (
    <div style={styles.panel}>
      ...
      <div style={styles.legendRow}>
        ...
        <LegendItem color={COLORS.CORRECTION_FREE} label={tr(lang, "تصحيح", "Correction")} value={0} />
      </div>
    </div>
  );
}
    ...
    <div dir={isRTL ? "rtl" : "ltr"} style={styles.page}>
      ...
      <div style={styles.container}>
            ...
            <div>
              ...
              <div style={styles.heroFeatureRow}>
                ...
                <div style={styles.heroFeatureCard}>
                  ...
                  <div style={styles.heroFeatureLabel}>{tr(lang, "مهمة فعلية", "Real tasks")}</div>
                </div>
                <div style={styles.heroFeatureCard}>
        ...
        <div style={styles.kpiGrid}>
          ...
          <KpiCard title={tr(lang, "إجمالي المهام", "Total tasks")} value={totalTasks} subtitle={tr(lang, "كل أنواع التكليفات", "All assignment types")} />
                  ...
                  <div key={row.teacher} style={styles.teacherCard}>
                    <div style={styles.teacherHeader}>
                      <div>
                        ...
                        <div style={styles.teacherSub}>{tr(lang, "إجمالي الحمل", "Total load")}: {row.total}</div>
                      </div>
