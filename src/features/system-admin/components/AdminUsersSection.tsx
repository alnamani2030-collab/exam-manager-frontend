...
function isSchoolRole(role: string) {
  const r = String(role || "").trim().toLowerCase();
  return r === "tenant_admin" || r === "admin" || r === "user";
}

function isGovernorateRole(role: string) {
  const r = String(role || "").trim().toLowerCase();
  return r === "super" || r === "super_regional" || r === "regional_super";
}

function isMinistryRole(role: string) {
  return String(role || "").trim().toLowerCase() === "ministry_super";
}
    ...
    >
      <Card title="إضافة/ربط مستخدم بالمدرسة أو الصلاحيات العليا">
        <div style={{ display: "grid", gap: 10 }}>
          ...
          {isMinistryRole(newUserRole) ? (
            <div style={{ fontSize: 13, opacity: 0.85, lineHeight: 1.8 }}>
              النطاق سيتم تثبيته تلقائيًا على: <b style={{ color: GOLD }}>{MINISTRY_SCOPE}</b>
            </div>
          ) : null}
          ...
          <Button onClick={createAllowUser} disabled={!canCreateUser || !canAssignNewUserRole}>
            حفظ المستخدم
          </Button>

          <div style={{ fontSize: 12, opacity: 0.85, lineHeight: 1.8 }}>
            <b style={{ color: GOLD }}>تنبيه:</b>
            <br />
            هذه الشاشة أصبحت تعرض وتميّز بين:
            <br />
            - مالك المنصة
            <br />
            - سوبر الوزارة
      ...
      <Card
        title="قائمة المستخدمين (allowlist)"
        right={
          <Input
            value={search}
            onChange={(e) => setSearch(e.target.value)}
            placeholder="بحث: email / tenant / governorate / role..."
            style={{ width: 360 }}
          />
        }
      >
          ...
          <table style={{ width: "100%", borderCollapse: "separate", borderSpacing: "0 10px" }}>
            ...
            <tbody>
              {filteredUsers.map((u: any) => {
                ...
                const currentRole = String(view.role || "").trim().toLowerCase();
                  ...
                  >
                    ...
                    >
                      ...
                      <div style={{ fontSize: 12, opacity: 0.85 }}>
                        {u.userName || u.name || u.schoolName || u.tenantName || ""}
                      </div>
                    </td>
                    ...
                    <td style={{ padding: "12px 10px", verticalAlign: "top" }}>
                      <select
                        ...
                        onChange={(e) => {
                          ...
                          if (isMinistryRole(nextRole)) {
                            patch.governorate = MINISTRY_SCOPE;
                          } else if (!isGovernorateRole(nextRole)) {
                            patch.governorate = "";
                          }

                          if (!isSchoolRole(nextRole)) {
                            patch.tenantId = "";
                          }
                    ...
                    <td style={{ padding: "12px 10px", verticalAlign: "top" }}>
                      {isSchoolRole(currentRole) ? (
                        ...
                        <div style={{ opacity: 0.8, fontSize: 13 }}>
                          {view.tenantId || "غير مرتبط بمدرسة"}
                        </div>
                      )}
                    </td>

                    <td style={{ padding: "12px 10px", verticalAlign: "top" }}>
                      {isGovernorateRole(currentRole) ? (
                        <select
                          value={view.governorate || ""}
                          disabled={isPrimaryRow || !canManageRowRole}
                          onChange={(e) => {
                            if (isPrimaryRow || !canManageRowRole) return;
                            setDraft(u.email, { governorate: e.target.value });
                          }}
                          style={{
                            padding: "10px 12px",
                            borderRadius: 12,
                        ...
                        <div style={{ opacity: 0.8, fontSize: 13 }}>
                          {view.governorate || view.tenantGovernorate || "-"}
                        </div>
                      )}
                    </td>
                    ...
                    >
                      <div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
                        <Button
                          variant="ghost"
                          disabled={!dirty || isPrimaryRow || !canManageRowRole}
                          onClick={async () => {
                          ...
                          title={dirty ? "تطبيق التعديل" : "لا يوجد تعديل"}
                        >
                          تعديل
                        </Button>
                        <Button
