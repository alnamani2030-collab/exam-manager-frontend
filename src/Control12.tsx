...
const normalizeEmailForGate = (value: string) => String(value || "").trim().toLowerCase();
...
export default function SchoolControl() {
  ...
  const [accessConfirmEmail, setAccessConfirmEmail] = useState("");
  const [accessEmailConfirmed, setAccessEmailConfirmed] = useState(false);
  ...
  useEffect(() => {
    ...
    setAccessGateError("");
    setAccessConfirmEmail("");
    setAccessEmailConfirmed(false);
  }, [accessSessionKey]);
  ...
  const sendControlAccessCode = async () => {
    ...
    const expectedEmail = normalizeEmailForGate(String(user?.email || ""));
    const enteredEmail = normalizeEmailForGate(accessConfirmEmail);

    if (!expectedEmail) {
      setAccessEmailConfirmed(false);
      setAccessGateError(tr("البريد الإلكتروني المسجل للحساب غير متوفر.", "The account email is unavailable."));
      return;
    }
    ...
    if (!enteredEmail || enteredEmail !== expectedEmail) {
      ...
      setAccessCodeSent(false);
      setAccessCode("");
      setAccessGateError(tr("البريد الإلكتروني غير مطابق للحساب الحالي. لن يتم إرسال رمز الدخول.", "The email does not match the current account. The access code will not be sent."));
      return;
    }

    setAccessEmailConfirmed(true);
      ...
      >
        ...
        >
          <div style={{ textAlign: "center", marginBottom: 18 }}>
            ...
            <div style={{ marginTop: 10, fontSize: 16, color: "#000000", fontWeight: 900 }}>
              {tr(
                ...
                "Enter the correct account email first, then request the access code sent to email."
              )}
            </div>
          </div>
          ...
          <div style={{ display: "grid", gap: 12 }}>
            <input
              value={accessConfirmEmail}
              onChange={(event) => {
                setAccessConfirmEmail(event.target.value);
                setAccessEmailConfirmed(false);
                setAccessCodeSent(false);
                setAccessCode("");
                setAccessGateMessage("");
            ...
            >
              {accessGateBusy ? tr("جار المعالجة...", "Processing...") : accessCodeSent ? tr("إعادة إرسال الرمز", "Resend code") : tr("تأكيد البريد وإرسال رمز الدخول", "Confirm email and send access code")}
            </button>

            {accessEmailConfirmed && accessCodeSent ? (
              <>
                <input
                  value={accessCode}
